...
// No imports here. Types like #Platform are inherited from the same package.

// Subset-of-platform schema: every path must exist in the base platform.
#PlatformOverrides: {
	// Nodes subset
	nodes?: [nodeName=string]: #Platform.nodes[nodeName] & {
		bridges?: [bridgeName=string]: #Platform.nodes[nodeName].bridges[bridgeName] & {
			vlan_ids?: [...int]
			mtu?:         int
			description?: string
			sdn_zones?: [...string]
	...
	// Storage subset
	storage?: [storageName=string]: #Platform.storage[storageName] & {
		type?: string
		content?: [...string]
		nodes?: [...string]
		lvm_thin?: {
